...
<div class="content-wrapper">
    <section class="content-header">
        <div class="container-fluid">
            <h1>Edit Setoran Tabungan Qurban</h1>
        </div>
    </section>
        ...
        <div class="container-fluid">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Form Edit Setoran</h3>
                </div>
                <div class="card-body">
                    <form action="{{ route('tabungan-qurban.update', $tabungan->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="nasabah_id">Nasabah</label>
                            <select class="form-control @error('nasabah_id') is-invalid @enderror"
                                    id="nasabah_id" name="nasabah_id" required>
                                <option value="">Pilih Nasabah</option>
                                @foreach($nasabah as $n)
                                <option value="{{ $n['id'] }}"
                                        data-target="{{ $n['target_harga'] }}"
                                        data-terkumpul="{{ $n['total_tabungan'] }}"
                                        data-sisa="{{ $n['sisa_tabungan'] }}"
                                        {{ $tabungan->nasabah_id == $n['id'] ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control @error('jumlah_setoran') is-invalid @enderror"
                                   id="jumlah_setoran" name="jumlah_setoran"
                                   value="{{ old('jumlah_setoran', $tabungan->jumlah_setoran) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" class="form-control @error('tanggal_setor') is-invalid @enderror"
                                   ...
                                   value="{{ old('tanggal_setor', $tabungan->tanggal_setor) }}" required>
                            @error('tanggal_setor')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('keterangan') is-invalid @enderror"
                                      id="keterangan" name="keterangan" rows="3">{{ old('keterangan', $tabungan->keterangan) }}</textarea>
                            @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
